...
int	write_no_such_file_or_dir(t_command *com, char *command)
{
...
char	*tmp_command(char *command, t_command *com)
{
	char	*command_tmp;
	int		i;

	command_tmp = ft_strdup(com->com);
...
int	bin_command(t_command *com, char **envp)
{
	...
	if (i != -1)
	{
		if (envp[i][4] == '=')
		{
			...
			if (check_file_before_exec(envp, com, command_tmp))
			{
				free(command_tmp);
				return (1);
			}
...
void	execve_command(t_command *com, char **arg, char **envp)
{
	...
	wait(&pid);
	if (ft_strlen(com->bin_exec.path))
